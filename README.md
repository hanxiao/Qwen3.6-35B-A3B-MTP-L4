...
5. **A lower quant *backfires* — it's slower, not faster (measured).** On the diverse benchmark, Q3_K_XL + ECC-off gives **MIN 86.9 tok/s (chat) — *below* Q4_K_XL's 91.7** — with MTP acceptance dropping 0.766 → 0.714. The lower-fidelity quant degrades the **MTP draft head**, so acceptance falls and the speculative speedup shrinks, outweighing the smaller (16 vs 22 GiB) weight reads. So dropping quant loses on *both* axes here: lower quality **and** lower throughput. (Raw non-MTP tg is higher for Q3, but real decode runs with MTP.)
...
**What actually reaches >100 tok/s:** *only* a higher-power / higher-bandwidth GPU — L40S (~864 GB/s), RTX 6000 Ada (~960 GB/s → ~240), or A100 (~2 TB/s). **Not** a quant change (MXFP4 has no FP4 hardware path on Ada/L4 — Blackwell-only; and Q3_K_XL is measurably *slower* than Q4 here because it degrades the MTP draft head) and **not** a kernel rewrite (the implemented MoE-verify fusion adds only +1%). On the single L4, ~92–93 (min) is the wall for this model.
...
7. **The L4 is the ceiling.** With ECC-off, ~92–100 tok/s across inputs (min ~92) is the honest decode limit for Q4_K_XL + MTP on a single, 72 W, 300 GB/s L4. 100-on-everything needs a faster GPU — and counter-intuitively a **lower quant makes it worse** (Q3_K_XL measured slower, because it degrades the MTP draft head → lower acceptance).
